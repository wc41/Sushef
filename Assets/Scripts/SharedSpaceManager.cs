namespace MyFirstARGame
{
    ...
    public class SharedSpaceManager : MonoBehaviour
    {
        ...
        private void TrackedImageController_Pressed(TrackedImageController sender, Vector3 position)
        {
            ...
            if (!this.clickedOnce && !this.clickedMoreThanOnce)
            {
                this.clickedOnce = true;
            } else
            {
                this.clickedMoreThanOnce = true;
            ...
            if (this.arCamera != null)
            {
                ...
                if (startLocationTrack)
                {
                    ...
                    if (!playerA && Vector3.Distance(new Vector3(0f, 0f, 1f), new Vector3(playerPosition.x, 0f, playerPosition.z)) < 0.3f)
                    {
                        ...
                        g.GetPhotonView().RPC("ReadyPlayer1", RpcTarget.MasterClient, workstation1.GetPhotonView().ViewID, PhotonNetwork.LocalPlayer.ActorNumber);
                        playerA = true;
                    }
                    ...
                    if (!playerB && Vector3.Distance(new Vector3(0f, 0f, -1f), new Vector3(playerPosition.x, 0f, playerPosition.z)) < 0.3f)
                    {
                        ...
                        g.GetPhotonView().RPC("ReadyPlayer2", RpcTarget.MasterClient, workstation2.GetPhotonView().ViewID, PhotonNetwork.LocalPlayer.ActorNumber);
                        playerB = true;
                    }
